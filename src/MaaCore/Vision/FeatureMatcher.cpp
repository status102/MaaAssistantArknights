...
std::pair<std::vector<cv::KeyPoint>, cv::Mat> asst::FeatureMatcher::detect(const cv::Mat& image,
...
std::vector<std::vector<cv::DMatch>> asst::FeatureMatcher::match(const cv::Mat& descriptors_templ,
                                                                 const cv::Mat& descriptors_image) const
{
...
std::vector<asst::FeatureMatcher::Result> asst::FeatureMatcher::postproc(
    const cv::Mat& templ, const std::vector<std::vector<cv::DMatch>>& match_points,
    ...
    const cv::Rect& roi_2) const
{
    ...
    Log.trace("Match:", good_matches.size(), ",match_points.size:", match_points.size(),
              ",m_params.distance_ratio:", m_params.distance_ratio);

    std::vector<Result> results;
    if (good_matches.size() >= 4) {
        cv::Mat H = cv::findHomography(obj, scene, cv::RANSAC);

        std::array<cv::Point2d, 4> obj_corners = { cv::Point2d(0, 0), cv::Point2d(templ.cols, 0),
                                                   cv::Point2d(templ.cols, templ.rows), cv::Point2d(0, templ.rows) };
...
void asst::FeatureMatcher::draw_result(const cv::Mat& templ, const std::vector<cv::KeyPoint>& keypoints_1,
                                       const cv::Rect& roi, const std::vector<cv::KeyPoint>& keypoints_2,
                                       const std::vector<cv::DMatch>& good_matches, std::vector<Result>& results) const
{
    ...
    for (size_t i = 0; i != results.size(); ++i) {
        ...
        cv::rectangle(image_draw, make_rect<cv::Rect>(res.rect), color, 1);
        /*
        ...
        cv::putText(image_draw, flag, cv::Point(res.rect.x, res.rect.y - 5), cv::FONT_HERSHEY_PLAIN, 1.2, color, 1);
    }
