...
namespace asst
{
    ...
    private:
        ...
        // match函数用于匹配两组特征描述符
        std::vector<std::vector<cv::DMatch>> match(const cv::Mat& descriptors_templ,
                                                   const cv::Mat& descriptors_image) const;
        // postproc函数用于对匹配结果进行后处理
        std::vector<Result> postproc(const cv::Mat& templ,
                                     const std::vector<std::vector<cv::DMatch>>& match_points,
                                     const std::vector<cv::KeyPoint>& keypoints_1, const std::vector<cv::KeyPoint>& keypoints_2, const cv::Rect& roi_2) const;
